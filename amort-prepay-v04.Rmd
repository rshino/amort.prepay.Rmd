...
cfm<-as.matrix(cfloan[,c("int","prin")])
rownames(cfm)<-cfloan$mon
cfmt<-t(cfm)
barplot(cfmt,border=NA,col=c("blue","red"),space=c(0,0))

cprsingle<-replicate(orig.term,0.01)
cfloan<-amort.loan.cf(orig.term,note.rate,orig.bal,cprsingle)
cfm<-as.matrix(cfloan[,c("int","prin")])
rownames(cfm)<-cfloan$mon
cfmt<-t(cfm)
