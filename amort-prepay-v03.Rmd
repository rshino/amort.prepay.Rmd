...
* Conditional Prepayment Rate (CPR) = `r sprintf("%5.2f%%",scalar.cpr*100)`
...
Other characteristics of the single loan:
...
### Amortize Pool by MBS Convention
...
CPRs for a pool are treated as full paydown of a portion of the pool. 
There is no provision for curtailment.

#### Example: 

1. Assume flat 6% CPR

2. 6% CPR is annualized equivalent of 0.51% Single Monthly Mortality (SMM).

3. Each month, 0.51% of the loans pay in full

4. The scheduled level payment is reduced by 0.51% reflecting the paydowns.
...
### Simulate Pool Paydowns

The simulation models a pool as an aggregate of a finite number of realistically-sized loans, which may have heterogeneous properties (different coupons, sizes, etc).

CPRs represent the probability for each loan being paid in full. 
...
4. The scheduled level payment is reduced by those loans paying in full.
